...
namespace lemon{namespace rc{namespace tools{
	...
	{
		_package.error_message(code,lemon::from_locale(name),lemon::from_locale(descripton));
	}

	void AssemblyInfo::AddTraceCatalog(lemon::uint32_t value,const char * name,const char * descripton)
	{
		_package.trace_catalog(value,lemon::from_locale(name),lemon::from_locale(descripton));
	}
	...
	void AssemblyInfo::AddTraceEvent(lemon::uint32_t seq,const std::string & key)
	{
		...
		_package.trace(seq,lemon::from_locale(key));
	}
